...
class RodCommentScreen extends StatelessWidget {
  final Rod rod;

  const RodCommentScreen({Key? key, required this.rod}) : super(key: key);
    ...
    return Scaffold(
      ...
      body: Container(
        ...
        child: ListView.builder(
          itemCount: rod.rodComments.length,
          itemBuilder: (context, index) {
            final comment = rod.rodComments[index];
            return Card(
                  ...
                  children: [
                    Row(
                      children: [
                        ...
                        const SizedBox(width: 10),
                        Text(
                          _getStars(comment.rating.toInt()), // Звёздочки вместо текста
                          style: const TextStyle(
                            fontSize: 24, // Увеличенный размер шрифта для звёздочек
